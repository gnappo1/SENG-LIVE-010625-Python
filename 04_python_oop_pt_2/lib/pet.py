...
class Pet:
    ...
    def __init__(self, name, age, breed, temperament, owner):
        ...
        self.owner = owner
        # Pet.all.append(self) #! BAD for inheritance, it breaks
        # self.all.append(self) #! works in python because objects know about class' attributes
        type(self).all.append(self) #* IDEAL
    ...
    @classmethod
    def find_by_name_and_owner(cls, name, owner):
        # for pet in cls.all:
        #     if pet.name == name and pet._owner == owner:
        #         return pet
        return next((pet for pet in cls.all if pet.name == name and pet._owner == owner), None)

    @classmethod
    def sort_by_age(cls):
        return sorted(cls.all, key=lambda pet: pet.age, reverse= True)
...
milo = Pet(name="Milo", age=1, breed="pug", temperament="docile", owner="Matteo")
